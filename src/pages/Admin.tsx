    ...
    return (
      <div className="min-h-screen bg-neutral-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl border border-neutral-200 shadow-sm p-8 w-full max-w-sm">
          ...
          <input type="password" placeholder="Admin key" value={keyInput} onChange={(e) => setKeyInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && login()} className="w-full rounded-lg border border-neutral-300 px-3 py-2 mb-4 text-sm outline-none focus:border-orange-500" />
    ...
    <div className="min-h-screen bg-neutral-50 flex">
      <aside className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-neutral-200 transition-transform lg:translate-x-0 lg:static`}>
        ...
        <nav className="p-2 space-y-1">
          {tabs.map((t) => (
            <button key={t.id} onClick={() => { setTab(t.id); setSidebarOpen(false) }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition ${tab === t.id ? "bg-neutral-100 text-neutral-900" : "text-neutral-600 hover:bg-neutral-50"}`}>
    ...
    <div className="space-y-4 p-4">
      <div className="flex gap-2 flex-wrap">
        <input 
          ...
          className="flex-1 min-w-[200px] rounded-lg border border-neutral-300 px-3 py-2 text-sm outline-none focus:border-orange-500"
        />
        <select 
          value={statusFilter} 
