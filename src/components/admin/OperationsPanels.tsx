import { Bell, CheckCircle2, Clock3, CreditCard, Crown, FileWarning, Mail, Megaphone, MessageCircle, TrendingUp } from 'lucide-react'
...
export function NotificationsPanel({ adminKey }: { adminKey: string }) {
  ...
  const subscribers = trpc.admin.marketingSubscribers.useQuery({ key: adminKey })
  ...
  const subscriberData = (subscribers.data as any) ?? {}
  const totals = subscriberData.totals ?? { email: 0, whatsapp: 0 }
  const [marketing, setMarketing] = useState<{ configured: boolean; provider: string | null; from: string | null; campaigns: any[]; error: string }>({ configured: false, provider: null, from: null, campaigns: [], error: '' })
  ...
  useEffect(() => {
    ...
    ]).then(async ([statusResponse, campaignResponse]) => {
      const status = await statusResponse.json().catch(() => ({}))
      const campaigns = await campaignResponse.json().catch(() => [])
      if (cancelled) return
      if (!statusResponse.ok || !campaignResponse.ok) {
        setMarketing((current) => ({ ...current, error: status.error || 'Marketing delivery status could not load.' }))
        return
      }
      setMarketing({ configured: Boolean(status.configured), provider: status.provider ?? null, from: status.from ?? null, campaigns: Array.isArray(campaigns) ? campaigns : [], error: '' })
    }).catch(() => !cancelled && setMarketing((current) => ({ ...current, error: 'Marketing delivery status could not load.' })))
  ...
  return <div className="space-y-4">
    <section className="rounded-xl border border-neutral-200 bg-white p-4">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className={`rounded-xl p-3 ${marketing.configured ? 'bg-emerald-50 text-emerald-800' : 'bg-amber-50 text-amber-900'}`}><Mail size={17} /><p className="mt-2 font-black">{marketing.configured ? 'Connected' : 'Not configured'}</p><p className="text-xs">Email delivery{marketing.provider ? ` · ${marketing.provider}` : ''}</p></div>
        <div className="rounded-xl bg-slate-100 p-3 text-slate-700"><MessageCircle size={17} /><p className="mt-2 font-black">Not connected</p><p className="text-xs">WhatsApp/SMS delivery</p></div>
      </div>
      {marketing.configured && marketing.from && <p className="mt-3 text-xs text-neutral-500">Verified sender: {marketing.from}</p>}
      {marketing.error && <p className="mt-3 rounded-lg bg-rose-50 p-3 text-xs font-semibold text-rose-700">{marketing.error}</p>}
      <div className="mt-4"><p className="text-xs font-black uppercase tracking-wide text-neutral-500">Recent campaigns</p>{recentCampaigns.length === 0 ? <p className="mt-2 text-sm text-neutral-500">No saved marketing campaigns yet.</p> : <div className="mt-2 grid gap-2 md:grid-cols-2">{recentCampaigns.map((campaign: any) => <div key={campaign.id} className="rounded-lg border border-neutral-200 p-3"><div className="flex items-start justify-between gap-2"><p className="text-sm font-bold">{campaign.name}</p><span className="rounded-full bg-neutral-100 px-2 py-1 text-[10px] font-black uppercase">{campaign.status}</span></div><p className="mt-1 text-xs text-neutral-500">{campaign.sentAt ? `Sent ${campaign.sentCount || 0} · Failed ${campaign.failedCount || 0}` : campaign.scheduledFor ? `Scheduled ${new Date(campaign.scheduledFor).toLocaleString('en-UG')}` : 'Draft'}</p>{campaign.lastError && <p className="mt-1 text-xs font-semibold text-rose-700">{campaign.lastError}</p>}</div>)}</div>}</div>
    </section>
    <div className="flex items-center justify-between rounded-xl border border-neutral-200 bg-white p-4"><div><h2 className="flex items-center gap-2 font-bold"><Bell size={18} /> Operational notifications</h2><p className="mt-1 text-xs text-neutral-500">Internal order, seller, delivery and payout events. These are not customer marketing messages.</p></div><button onClick={() => markAll.mutate({ key: adminKey })} disabled={markAll.isPending || rows.every((row: any) => row.isRead)} className="rounded-lg bg-neutral-900 px-3 py-2 text-xs font-bold text-white disabled:opacity-40">Mark all read</button></div>
